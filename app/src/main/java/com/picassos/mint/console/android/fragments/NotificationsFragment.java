...
public class NotificationsFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        view.findViewById(R.id.notifications_settings).setOnClickListener(v -> {
            ...
            notificationsPreferencesBottomSheetModal.show(requireFragmentManager(), "TAG");
        });

        // notification details
        notificationTitle = view.findViewById(R.id.notification_title);
        notificationBody = view.findViewById(R.id.notification_body);
        notificationTarget = view.findViewById(R.id.notification_target);
        notificationIcon = view.findViewById(R.id.notification_icon);
        notificationImage = view.findViewById(R.id.notification_image);

        // open notification target chooser
        view.findViewById(R.id.notification_target_chooser).setOnClickListener(v -> targetChooserDialog());
        ...
        sendNotification.setOnClickListener(v -> {
            ...
            if (!TextUtils.isEmpty(notificationTitle.getText().toString()) && !TextUtils.isEmpty(notificationBody.getText().toString()) && !notificationTarget.getText().toString().equals(getString(R.string.notification_target))) {
                if (consolePreferences.loadFirebaseAccessToken().equals("exception:error?fak")) {
                    Toasto.show_toast(requireContext(), getString(R.string.update_firebase_access_key), 1, 2);
                } else {
        ...
        view.findViewById(R.id.preview_notification).setOnClickListener(v -> {
            if (TextUtils.isEmpty(notificationTitle.getText().toString())) {
                notificationTitle.setError(getString(R.string.notification_title_required));
                notificationTitle.requestFocus();
                sendNotification.setEnabled(true);
    ...
    private void reviewNotification() {
        NotificationCompat.Builder notificationBuilder = new NotificationCompat.Builder(requireContext(), "notify_001")
            ...
            StringRequest request = new StringRequest(Request.Method.POST, API.API_URL + API.REQUEST_SEND_NOTIFICATION,
                    response -> {
                        if (response.contains("Request failed: HTTP status code:")) {
                            Toasto.show_toast(requireContext(), getString(R.string.invalid_firebase_access_key), 0, 2);
                        } else if (response.equals("200")) {
                            Toasto.show_toast(requireContext(), getString(R.string.notification_send), 0, 0);
                        }
    ...
    private void targetChooserDialog() {
        Dialog targetsDialog = new Dialog(requireContext());
        ...
        targetRecyclerview.setLayoutManager(new LinearLayoutManager(requireContext()));
